...
- **【裁定 2026-09-06】**：先前的珊瑚色实体拨杆方向已撤销。现有母版、方向板和工程内切图都只能作为历史材料，禁止复用其拨杆、紫色底座、两枚卡扣或工业硬件语义，也禁止在重新选定前接入工程。
- 新方向必须先交付一张无文字、无圆角遮罩、完整不透明背景的 3×3 方向板；每格从不同的可识别现实原型出发，且在核心隐喻、构图或视觉语言上实质不同。用户明确选中一个方向后，才可精绘独立母版、通过小尺寸可读性门禁并接入应用。
- **已选方向（2026-09-06）**：第 5 格「衣夹」。最终 App Icon 以一枚正在夹合的衣夹为唯一主轮廓，保留简洁的纸艺分层、薄荷绿背景、海军蓝上夹片与珊瑚色下夹片；禁止混入其它方向的字母、手势、卡扣、旋钮、铁路或日月符号。方向板切格仅作高保真重绘参考，禁止直接放大或接入工程。
